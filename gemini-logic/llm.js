"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _a, _b;
Object.defineProperty(exports, "__esModule", { value: true });
exports.analyzePersonality = analyzePersonality;
var generative_ai_1 = require("@google/generative-ai");
var fs = require("fs");
var dotenv = require("dotenv");
dotenv.config();
function analyzePersonality() {
    return __awaiter(this, void 0, void 0, function () {
        var apiKey, genAI, model, profilePath, postsPath, profile, posts, prompt, result, responseText, analysis, outputPath, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    apiKey = process.env.GEMINI_API_KEY;
                    if (!apiKey) {
                        throw new Error("GEMINI_API_KEY not found in environment variables.");
                    }
                    genAI = new generative_ai_1.GoogleGenerativeAI(apiKey);
                    model = genAI.getGenerativeModel({
                        model: "gemini-1.5-flash",
                        generationConfig: { responseMimeType: "application/json" },
                    });
                    profilePath = "./scraped-profile.json";
                    postsPath = "./scraped-posts.json";
                    if (!fs.existsSync(profilePath) || !fs.existsSync(postsPath)) {
                        throw new Error("Scraped data files not found. Please run the scraper first.");
                    }
                    profile = JSON.parse(fs.readFileSync(profilePath, "utf-8"));
                    posts = JSON.parse(fs.readFileSync(postsPath, "utf-8"));
                    prompt = "\n    Analyze the following LinkedIn profile and recent activity posts to create a detailed personality profile using the OCEAN (Big Five) model.\n    \n    ### OCEAN Model Definitions:\n    - Openness: Appreciation for art, emotion, adventure, unusual ideas, curiosity, and variety of experience.\n    - Conscientiousness: A tendency to be organized and dependable, show self-discipline, act dutifully, aim for achievement, and prefer planned rather than spontaneous behavior.\n    - Extraversion: Energy, positive emotions, surgency, assertiveness, sociability and the tendency to seek stimulation in the company of others.\n    - Agreeableness: A tendency to be compassionate and cooperative rather than suspicious and antagonistic towards others.\n    - Neuroticism: The tendency to experience unpleasant emotions easily, such as anger, anxiety, depression, and vulnerability.\n\n    ### LinkedIn Profile Data:\n    ".concat(JSON.stringify(profile, null, 2), "\n\n    ### Recent Post Activity:\n    ").concat(JSON.stringify(posts, null, 2), "\n\n    ### Task:\n    1. Score each OCEAN trait from 1-100.\n    2. Provide a description of why that score was given.\n    3. List 2-3 specific points of evidence from the profile or posts for each trait.\n    4. Provide an overall summary of the persona.\n\n    ### Output Format:\n    You must return a JSON object that strictly follows this structure:\n    {\n      \"openness\": { \"score\": number, \"description\": \"string\", \"evidence\": [\"string\"] },\n      \"conscientiousness\": { \"score\": number, \"description\": \"string\", \"evidence\": [\"string\"] },\n      \"extraversion\": { \"score\": number, \"description\": \"string\", \"evidence\": [\"string\"] },\n      \"agreeableness\": { \"score\": number, \"description\": \"string\", \"evidence\": [\"string\"] },\n      \"neuroticism\": { \"score\": number, \"description\": \"string\", \"evidence\": [\"string\"] },\n      \"overallPersonaSummary\": \"string\"\n    }\n  ");
                    console.log("Sending data to Gemini for OCEAN analysis...");
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, model.generateContent(prompt)];
                case 2:
                    result = _a.sent();
                    responseText = result.response.text();
                    analysis = JSON.parse(responseText);
                    outputPath = "./personality-analysis.json";
                    fs.writeFileSync(outputPath, JSON.stringify(analysis, null, 2));
                    console.log("✅ Personality analysis complete!");
                    console.log("Results saved to ".concat(outputPath));
                    return [2 /*return*/, analysis];
                case 3:
                    error_1 = _a.sent();
                    console.error("Error during personality analysis:", error_1);
                    throw error_1;
                case 4: return [2 /*return*/];
            }
        });
    });
}
// If running directly
if (((_a = process.argv[1]) === null || _a === void 0 ? void 0 : _a.endsWith("llm.ts")) ||
    ((_b = process.argv[1]) === null || _b === void 0 ? void 0 : _b.endsWith("llm.js"))) {
    analyzePersonality().catch(console.error);
}
